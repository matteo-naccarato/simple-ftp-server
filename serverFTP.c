...
void* listening(void*);
void* response(void*);
void invia(int connId, char*);
void ricevi(int connId, char buffer[MAX_PACK + 1]);
...
void* response(void* params) {
    ...
    // attesa USER
    ricevi(connId, buffer);
    printf("[FTP] received from\t[client]\t'%s'\n", buffer);
    ...
    if (isUserValid(user)) {
        ...
        invia(connId, USER_OK_NEED_PASS);
        printf("[FTP] sent to\t\t[client]\t'%s'\n\n", USER_OK_NEED_PASS);
        ...
        free(tmp);
        if (isPassValid(pass)) {
            invia(connId, GREETING);
            printf("[FTP] sent to\t\t[client]\t'%s'\n\n", GREETING);

        } else invia(connId, USER_OR_PASS_WRONG);
        free(pass);
